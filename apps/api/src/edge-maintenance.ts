...
export type EdgeMaintenanceDependencies = {
  listEdges: (
    input: Required<Pick<EdgeListInput, "limit" | "sort" | "order">> & {
      q?: string
      cursor?: Cursor
    }
  ) => Promise<
...
export function registerEdgeMaintenanceRoutes(
  app: Hono<Env>,
  dependencies: EdgeMaintenanceDependencies
) {
  app.get("/api/v1/maintenance/edges", async (c) => {
    const input = parseCollection(c.req.url, false)
    if (input instanceof Response) return input
    const records = await dependencies.listEdges({
      ...input,
      limit: input.limit + 1,
    })
    return c.json(page(records, input), 200)
  })
  app.post("/api/v1/maintenance/edges", async (c) => {
    ...
    try {
      ...
      const data = serialize(result.edge)
      return c.json({ data }, 201, {
        ETag: data.etag,
        Location: `/api/v1/maintenance/edges/${data.id}`,
      })
    } catch (error) {
  ...
  app.all("/api/v1/maintenance/edges/:uuid", (c) =>
...
function parseCollection(url: string, optionsOnly: boolean): Input | Response {
  ...
  const parsed = (
    optionsOnly ? edgeOptionsInputSchema : edgeListInputSchema
  ).safeParse(raw)
...
async function parseBody(request: Request) {
  ...
  const parsed = edgeMutationBodySchema.safeParse(body)
  ...
  return problem(
    ...
    parsed.error.issues.map((issue) => {
      ...
      return {
        name: `/${field}`,
        code: `edge_${field}_${failure}`,
        reason:
          failure === "required"
            ? `Edge ${field === "code" ? "Code" : "Name"} must not be blank.`
            : failure === "too_long"
              ? `Edge ${field === "code" ? "Code" : "Name"} must contain at most 255 characters.`
              : `Edge ${field === "code" ? "Code" : "Name"} is invalid.`,
      }
    })
  )
}
function precondition(
  ...
  if (!value)
    return problem(
      ...
      "if_match_required",
      "If-Match required",
      "Edge replacement and deletion require an If-Match header",
      instance
    )
  try {
    ...
    return problem(
      ...
      "invalid_if_match",
      "Invalid If-Match",
      "If-Match does not identify this Edge version",
      instance
    )
  }
}
function mapError(
  ...
  if (constraint(error, "23505", "edge_code_lower_unique_idx"))
    return problem(
      409,
      "edge-code-conflict",
      "edge_code_conflict",
      "Edge Code already exists",
      "Another Edge already uses this Edge Code",
      instance
    )
  if (
    operation === "delete" &&
    constraint(error, "23001", "coin_edge_id_edge_id_fk")
  )
    return problem(
      409,
      "edge-in-use",
      "edge_in_use",
      "Edge is in use",
      "Coins still use this Edge, so it cannot be deleted",
      instance
    )
  throw error
}
...
function stale(instance: string) {
  return problem(
    ...
    "Edge changed",
    "The Edge changed after it was loaded; reload before retrying",
    instance
  )
}
