...
import type { Composition, CompositionListInput } from "@coin-archive/api"
import type { Hono } from "hono"

import type { MaintenanceCollector } from "./orientation-maintenance"

type Source = Omit<Composition, "createdAt" | "updatedAt" | "etag"> & {
  createdAt: Date
  updatedAt: Date
}
type Cursor = { value: string; secondaryValue: string; id: string }
export type CompositionMaintenanceDependencies = {
  listCompositions: (
    input: Required<Pick<CompositionListInput, "limit" | "sort" | "order">> & {
      q?: string
      cursor?: Cursor
    }
  ) => Promise<
  ...
  >
  getComposition: (id: string) => Promise<Source | null>
  createComposition: (input: {
    ...
    | { status: "created" | "replayed"; composition: Source }
    | { status: "mismatch" }
  >
  replaceComposition: (input: {
    id: string
    expectedVersion: number
    fields: { code: string; name: string }
  }) => Promise<
    { status: "updated"; composition: Source } | { status: "missing" | "stale" }
  >
  deleteComposition: (input: {
    id: string
    expectedVersion: number
  }) => Promise<
    { status: "deleted"; composition: Source } | { status: "missing" | "stale" }
  >
}
type Env = { Variables: { collector: MaintenanceCollector; requestId: string } }

export function registerCompositionMaintenanceRoutes(
  app: Hono<Env>,
  dependencies: CompositionMaintenanceDependencies
) {
  app.get("/api/v1/maintenance/compositions", async (c) => {
    const input = parseCollection(c.req.url, false)
    if (input instanceof Response) return input
    const records = await dependencies.listCompositions({
      ...input,
      limit: input.limit + 1,
    })
    return c.json(page(records, input), 200)
  })
  app.post("/api/v1/maintenance/compositions", async (c) => {
    ...
    if (!key)
      return problem(
        ...
        "Idempotency-Key required",
        "Composition create requires an Idempotency-Key header",
        c.req.path
      )
    if (key.length > 255)
      return problem(
    ...
    try {
      const result = await dependencies.createComposition({
      ...
      const data = serialize(result.composition)
      return c.json({ data }, 201, {
        ETag: data.etag,
        Location: `/api/v1/maintenance/compositions/${data.id}`,
      })
    } catch (error) {
      return mapError(error, "create", c.req.path)
    }
  })
  app.all("/api/v1/maintenance/compositions", (c) =>
    method(c.req.path, "GET, POST")
  )
  app.get("/api/v1/maintenance/compositions/options", async (c) => {
  ...
  app.all("/api/v1/maintenance/compositions/options", (c) =>
    method(c.req.path, "GET")
  )
  app.get("/api/v1/maintenance/compositions/:uuid", async (c) => {
    const id = c.req.param("uuid")
    if (!uuid(id)) return invalidId(c.req.path)
    const record = await dependencies.getComposition(id)
    if (!record) return notFound(c.req.path)
    return c.json({ data: serialize(record) }, 200, { ETag: etag(record) })
  })
  app.put("/api/v1/maintenance/compositions/:uuid", async (c) => {
    const id = c.req.param("uuid")
    if (!uuid(id)) return invalidId(c.req.path)
    ...
    if (fields instanceof Response) return fields
    try {
      const result = await dependencies.replaceComposition({
        id,
  ...
  app.delete("/api/v1/maintenance/compositions/:uuid", async (c) => {
    const id = c.req.param("uuid")
    if (!uuid(id)) return invalidId(c.req.path)
    const version = precondition(c.req.header("if-match"), id, c.req.path)
    if (version instanceof Response) return version
    try {
      const result = await dependencies.deleteComposition({
        id,
        expectedVersion: version,
      })
  ...
  app.all("/api/v1/maintenance/compositions/:uuid", (c) =>
...
function parseCollection(url: string, optionsOnly: boolean): Input | Response {
  ...
  const parsed = (
    optionsOnly ? compositionOptionsInputSchema : compositionListInputSchema
  ).safeParse(raw)
...
async function parseBody(request: Request) {
  ...
  const parsed = compositionMutationBodySchema.safeParse(body)
  ...
  return problem(
    ...
    parsed.error.issues.map((issue) => {
      ...
      return {
        name: `/${field}`,
        code: `composition_${field}_${failure}`,
        reason:
          failure === "required"
            ? `Composition ${field === "code" ? "Code" : "Name"} must not be blank.`
            : failure === "too_long"
              ? `Composition ${field === "code" ? "Code" : "Name"} must contain at most 255 characters.`
              : `Composition ${field === "code" ? "Code" : "Name"} is invalid.`,
      }
    })
  )
}
function precondition(
  ...
  if (!value)
    return problem(
      ...
      "if_match_required",
      "If-Match required",
      "Composition replacement and deletion require an If-Match header",
      instance
    )
  try {
    ...
    return problem(
      ...
      "invalid_if_match",
      "Invalid If-Match",
      "If-Match does not identify this Composition version",
      instance
    )
  }
}
function mapError(
  ...
  if (constraint(error, "23505", "composition_code_lower_unique_idx"))
    return problem(
      409,
      "composition-code-conflict",
      "composition_code_conflict",
      "Composition Code already exists",
      "Another Composition already uses this Composition Code",
      instance
    )
  if (
    operation === "delete" &&
    constraint(error, "23001", "coin_composition_id_composition_id_fk")
  )
    return problem(
      409,
      "composition-in-use",
      ...
      "Coins still use this Composition, so it cannot be deleted",
      instance
    )
  throw error
}
...
function invalidId(instance: string) {
  return problem(
    400,
    "invalid-composition-uuid",
    "invalid_composition_uuid",
    "Invalid Composition UUID",
    "Composition UUID is invalid",
    instance
  )
}
function notFound(instance: string) {
  return problem(
    ...
    "Composition not found",
    "No Composition matches this UUID",
    instance
  )
}
function stale(instance: string) {
  return problem(
    412,
    "stale-composition",
    "composition_precondition_failed",
    "Composition changed",
    "The Composition changed after it was loaded; reload before retrying",
    instance
  )
}
