...
describe("edge-maintenance public API", () => {
  ...
  it("keeps migrated Edge reads and writes outside the database package boundary", () => {
    for (const file of ["actions.ts", "edge-maintenance-route-data.ts"]) {
    ...
    expect(coinFormSource).toContain("maintenanceClient.edges.options")
