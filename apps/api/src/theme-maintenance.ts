...
import type { Theme, ThemeListInput } from "@coin-archive/api"
import type { Hono } from "hono"

import type { MaintenanceCollector } from "./orientation-maintenance"

type ThemeSourceRecord = Omit<Theme, "createdAt" | "updatedAt" | "etag"> & {
  createdAt: Date
  updatedAt: Date
}
type Cursor = { value: string; secondaryValue: string; id: string }
export type ThemeMaintenanceDependencies = {
  listThemes: (
    input: Required<Pick<ThemeListInput, "limit" | "sort" | "order">> & {
      q?: string
      cursor?: Cursor
    }
  ) => Promise<
    (ThemeSourceRecord & { cursorValue: string; cursorSecondaryValue: string })[]
  >
  getTheme: (id: string) => Promise<ThemeSourceRecord | null>
  createTheme: (input: {
    collectorId: string
    ...
    | { status: "created" | "replayed"; theme: ThemeSourceRecord }
    | { status: "mismatch" }
  >
  replaceTheme: (input: {
    id: string
    ...
    | { status: "updated"; theme: ThemeSourceRecord }
  ...
  deleteTheme: (input: {
    ...
    | { status: "deleted"; theme: ThemeSourceRecord }
    | { status: "missing" | "stale" }
  >
}
type Env = { Variables: { collector: MaintenanceCollector; requestId: string } }

export function registerThemeMaintenanceRoutes(
  app: Hono<Env>,
  dependencies: ThemeMaintenanceDependencies
) {
  app.get("/api/v1/maintenance/themes", async (c) => {
    const input = parseCollection(c.req.url, false)
    if (input instanceof Response) return input
    const records = await dependencies.listThemes({
      ...input,
      limit: input.limit + 1,
    })
    return c.json(page(records, input), 200)
  })
  app.post("/api/v1/maintenance/themes", async (c) => {
    ...
    if (!key)
      return problem(
        ...
        "Idempotency-Key required",
        "Theme create requires an Idempotency-Key header",
        c.req.path
      )
    const parsedHeaders = themeCreateInputSchema.shape.headers.safeParse({
      "idempotency-key": key,
    })
    ...
    const input = themeCreateInputSchema.parse({
      headers: parsedHeaders.data,
      body: fields,
    })
    try {
      const result = await dependencies.createTheme({
      ...
      const data = serialize(result.theme)
      return c.json({ data }, 201, {
        ETag: data.etag,
        Location: `/api/v1/maintenance/themes/${data.id}`,
      })
    } catch (error) {
      return mapError(error, "create", c.req.path)
    }
  })
  app.all("/api/v1/maintenance/themes", (c) => method(c.req.path, "GET, POST"))
  app.get("/api/v1/maintenance/themes/options", async (c) => {
  ...
  app.all("/api/v1/maintenance/themes/options", (c) => method(c.req.path, "GET"))
  app.get("/api/v1/maintenance/themes/:uuid", async (c) => {
    const id = c.req.param("uuid")
    if (!themeDetailInputSchema.safeParse({ uuid: id }).success)
      return invalidId(c.req.path)
    const record = await dependencies.getTheme(id)
    if (!record) return notFound(c.req.path)
    return c.json({ data: serialize(record) }, 200, { ETag: etag(record) })
  })
  app.put("/api/v1/maintenance/themes/:uuid", async (c) => {
    const id = c.req.param("uuid")
    if (!themeReplaceInputSchema.shape.params.safeParse({ uuid: id }).success)
      return invalidId(c.req.path)
    const version = precondition(c.req.header("if-match"), id, c.req.path)
    if (version instanceof Response) return version
    const fields = await parseBody(c.req.raw)
    if (fields instanceof Response) return fields
    const input = themeReplaceInputSchema.parse({
      params: { uuid: id },
      headers: { "if-match": c.req.header("if-match") },
      body: fields,
    })
    try {
      const result = await dependencies.replaceTheme({
  ...
  app.delete("/api/v1/maintenance/themes/:uuid", async (c) => {
    const id = c.req.param("uuid")
    if (!themeDeleteInputSchema.shape.params.safeParse({ uuid: id }).success)
      return invalidId(c.req.path)
    const version = precondition(c.req.header("if-match"), id, c.req.path)
    if (version instanceof Response) return version
    const input = themeDeleteInputSchema.parse({
      params: { uuid: id },
      headers: { "if-match": c.req.header("if-match") },
    })
    try {
      const result = await dependencies.deleteTheme({
        id: input.params.uuid,
        expectedVersion: version,
      })
  ...
  app.all("/api/v1/maintenance/themes/:uuid", (c) =>
...
function parseCollection(
  ...
  const parsed = (
    optionsOnly ? themeOptionsInputSchema : themeListInputSchema
  ).safeParse(raw)
  if (!parsed.success) return invalidQuery(requestUrl.pathname)
  const sort = optionsOnly
      ? "name"
      : ((raw.sort as ThemeCollectionInput["sort"] | undefined) ?? "name"),
    order = optionsOnly
      ? "asc"
      : ((raw.order as ThemeCollectionInput["order"] | undefined) ?? "asc")
  const cursor =
...
function page(
  ...
  input: Pick<ThemeCollectionInput, "limit" | "sort" | "order">
) {
...
async function parseBody(request: Request) {
  ...
  const parsed = themeMutationBodySchema.safeParse(body)
    ...
    parsed.error.issues.map((issue) => {
      ...
      if (pathField !== "code" && pathField !== "name") {
        return {
          ...
          reason: "The Theme request body must be an object.",
        }
      }
      ...
      return {
        name: `/${field}`,
        code: `theme_${field}_${failure}`,
        reason:
          failure === "required"
            ? `Theme ${field === "code" ? "Code" : "Name"} must not be blank.`
            : failure === "too_long"
              ? `Theme ${field === "code" ? "Code" : "Name"} must contain at most 255 characters.`
              : `Theme ${field === "code" ? "Code" : "Name"} is invalid.`,
      }
    })
  )
}
function precondition(
  ...
  if (!value)
    return problem(
      ...
      "if_match_required",
      "If-Match required",
      "Theme replacement and deletion require an If-Match header",
      instance
    )
  try {
    ...
    return problem(
      ...
      "invalid_if_match",
      "Invalid If-Match",
      "If-Match does not identify this Theme version",
      instance
    )
  }
}
function mapError(
  ...
  if (constraint(error, "23505", "theme_code_lower_unique_idx"))
    return problem(
      409,
      "theme-code-conflict",
      "theme_code_conflict",
      "Theme Code already exists",
      "Another Theme already uses this Theme Code",
      instance
    )
  if (
    operation === "delete" &&
    ...
    return problem(
      ...
      "theme_in_use",
      "Theme is in use",
      "Theme Attributions still use this Theme, so it cannot be deleted",
      instance
    )
  throw error
}
...
function stale(instance: string) {
  return problem(
    ...
    "Theme changed",
    "The Theme changed after it was loaded; reload before retrying",
    instance
  )
}
...
function etag(record: Pick<ThemeSourceRecord, "id" | "version">) {
