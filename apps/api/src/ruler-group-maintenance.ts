...
type RulerGroupSourceRecord = Omit<
  RulerGroup,
...
export type RulerGroupMaintenanceDependencies = {
  listRulerGroups: (
    input: Required<
      Pick<RulerGroupListInput, "limit" | "sort" | "order">
    > & {
      q?: string
      cursor?: Cursor
    }
  ) => Promise<
    (RulerGroupSourceRecord & {
      cursorValue: string
  ...
  getRulerGroup: (
    id: string
  ) => Promise<RulerGroupSourceRecord | null>
  createRulerGroup: (input: {
    ...
    | {
        ...
        rulerGroup: RulerGroupSourceRecord
      }
    | { status: "mismatch" }
  >
  replaceRulerGroup: (input: {
    id: string
    expectedVersion: number
    fields: { code: string; name: string }
  }) => Promise<
    | { status: "updated"; rulerGroup: RulerGroupSourceRecord }
  ...
  deleteRulerGroup: (input: {
    ...
    | { status: "deleted"; rulerGroup: RulerGroupSourceRecord }
    | { status: "missing" | "stale" }
  >
}
type Env = { Variables: { collector: MaintenanceCollector; requestId: string } }

export function registerRulerGroupMaintenanceRoutes(
  app: Hono<Env>,
  dependencies: RulerGroupMaintenanceDependencies
) {
  app.get("/api/v1/maintenance/ruler-groups", async (c) => {
    const input = parseCollection(c.req.url, false)
    if (input instanceof Response) return input
    const records = await dependencies.listRulerGroups({
      ...input,
      limit: input.limit + 1,
    })
    return c.json(page(records, input), 200)
  })
  app.post("/api/v1/maintenance/ruler-groups", async (c) => {
    ...
    if (!key)
      return problem(
        ...
        "Idempotency-Key required",
        "Ruler Group create requires an Idempotency-Key header",
        c.req.path
      )
    const parsedHeaders =
      rulerGroupCreateInputSchema.shape.headers.safeParse({
        "idempotency-key": key,
      })
    ...
    const input = rulerGroupCreateInputSchema.parse({
      headers: parsedHeaders.data,
      body: fields,
    })
    try {
      const result = await dependencies.createRulerGroup({
      ...
      const data = serialize(result.rulerGroup)
      return c.json({ data }, 201, {
        ETag: data.etag,
        Location: `/api/v1/maintenance/ruler-groups/${data.id}`,
      })
    } catch (error) {
      return mapError(error, "create", c.req.path)
    }
  })
  app.all("/api/v1/maintenance/ruler-groups", (c) =>
    method(c.req.path, "GET, POST")
  )
  app.get("/api/v1/maintenance/ruler-groups/options", async (c) => {
  ...
  app.all("/api/v1/maintenance/ruler-groups/options", (c) =>
    method(c.req.path, "GET")
  )
  app.get("/api/v1/maintenance/ruler-groups/:uuid", async (c) => {
    const id = c.req.param("uuid")
    if (!rulerGroupDetailInputSchema.safeParse({ uuid: id }).success)
      return invalidId(c.req.path)
    const record = await dependencies.getRulerGroup(id)
    if (!record) return notFound(c.req.path)
    return c.json({ data: serialize(record) }, 200, { ETag: etag(record) })
  })
  app.put("/api/v1/maintenance/ruler-groups/:uuid", async (c) => {
    const id = c.req.param("uuid")
    if (
      !rulerGroupReplaceInputSchema.shape.params.safeParse({ uuid: id })
        .success
    )
      return invalidId(c.req.path)
    ...
    const fields = await parseBody(c.req.raw)
    if (fields instanceof Response) return fields
    const input = rulerGroupReplaceInputSchema.parse({
      params: { uuid: id },
      headers: { "if-match": c.req.header("if-match") },
      body: fields,
    })
    try {
      const result = await dependencies.replaceRulerGroup({
  ...
  app.delete("/api/v1/maintenance/ruler-groups/:uuid", async (c) => {
    const id = c.req.param("uuid")
    if (
      !rulerGroupDeleteInputSchema.shape.params.safeParse({ uuid: id })
        .success
    )
      return invalidId(c.req.path)
    const version = precondition(c.req.header("if-match"), id, c.req.path)
    if (version instanceof Response) return version
    const input = rulerGroupDeleteInputSchema.parse({
      params: { uuid: id },
      headers: { "if-match": c.req.header("if-match") },
    })
    try {
      const result = await dependencies.deleteRulerGroup({
        id: input.params.uuid,
        expectedVersion: version,
      })
  ...
  app.all("/api/v1/maintenance/ruler-groups/:uuid", (c) =>
    method(c.req.path, "GET, PUT, DELETE")
  )
}

type RulerGroupCollectionInput = {
  q?: string
  cursor?: Cursor
  limit: number
  sort: "code" | "name"
...
function parseCollection(
  url: string,
  optionsOnly: boolean
): RulerGroupCollectionInput | Response {
  ...
  const sort = optionsOnly
      ? "name"
      : ((raw.sort as RulerGroupCollectionInput["sort"] | undefined) ??
        "name"),
    order = optionsOnly
      ? "asc"
      : ((raw.order as RulerGroupCollectionInput["order"] | undefined) ??
        "asc")
  const cursor =
...
function page(
  records: (RulerGroupSourceRecord & {
    cursorValue: string
    cursorSecondaryValue: string
  })[],
  input: Pick<RulerGroupCollectionInput, "limit" | "sort" | "order">
) {
...
async function parseBody(request: Request) {
  ...
  const parsed = rulerGroupMutationBodySchema.safeParse(body)
  if (parsed.success) return parsed.data
  return problem(
    422,
    "ruler-group-validation",
    "ruler_group_validation_failed",
    "Ruler Group validation failed",
    "The Ruler Group could not be saved",
    new URL(request.url).pathname,
    parsed.error.issues.map((issue) => {
      ...
      if (pathField !== "code" && pathField !== "name") {
        return {
          ...
          reason: "The Ruler Group request body must be an object.",
        }
      }
      ...
      return {
        name: `/${field}`,
        code: `ruler_group_${field}_${failure}`,
        reason:
          failure === "required"
            ? `Ruler Group ${field === "code" ? "Code" : "Name"} must not be blank.`
            : failure === "too_long"
              ? `Ruler Group ${field === "code" ? "Code" : "Name"} must contain at most 255 characters.`
              : `Ruler Group ${field === "code" ? "Code" : "Name"} is invalid.`,
      }
    })
  )
}
function precondition(
  ...
  if (!value)
    return problem(
      ...
      "if_match_required",
      "If-Match required",
      "Ruler Group replacement and deletion require an If-Match header",
      instance
    )
  try {
    ...
    return problem(
      ...
      "invalid_if_match",
      "Invalid If-Match",
      "If-Match does not identify this Ruler Group version",
      instance
    )
  }
}
function mapError(
  ...
  if (constraint(error, "23505", "ruler_group_code_unique_idx"))
    return problem(
      409,
      "ruler-group-code-conflict",
      "ruler_group_code_conflict",
      "Ruler Group Code already exists",
      "Another Ruler Group already uses this Ruler Group Code",
      instance
    )
  if (
    operation === "delete" &&
    constraint(error, "23001", "ruler_ruler_group_id_ruler_group_id_fk")
  )
    return problem(
      409,
      "ruler-group-in-use",
      "ruler_group_in_use",
      "Ruler Group is in use",
      "Rulers still belong to this Ruler Group, so it cannot be deleted",
      instance
    )
  throw error
}
...
function invalidId(instance: string) {
  return problem(
    400,
    "invalid-ruler-group-uuid",
    "invalid_ruler_group_uuid",
    "Invalid Ruler Group UUID",
    "Ruler Group UUID is invalid",
    instance
  )
}
function notFound(instance: string) {
  return problem(
    ...
    "Ruler Group not found",
    "No Ruler Group matches this UUID",
    instance
  )
}
function stale(instance: string) {
  return problem(
    ...
    "Ruler Group changed",
    "The Ruler Group changed after it was loaded; reload before retrying",
    instance
  )
}
...
function etag(record: Pick<RulerGroupSourceRecord, "id" | "version">) {
