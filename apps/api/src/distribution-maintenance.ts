...
import type { Distribution, DistributionListInput } from "@coin-archive/api"
import type { Hono } from "hono"

import type { MaintenanceCollector } from "./orientation-maintenance"

type Source = Omit<Distribution, "createdAt" | "updatedAt" | "etag"> & {
  createdAt: Date
  updatedAt: Date
}
type Cursor = { value: string; secondaryValue: string; id: string }
export type DistributionMaintenanceDependencies = {
  listDistributions: (
    input: Required<Pick<DistributionListInput, "limit" | "sort" | "order">> & {
      q?: string
      cursor?: Cursor
    }
  ) => Promise<
  ...
  >
  getDistribution: (id: string) => Promise<Source | null>
  createDistribution: (input: {
    ...
    | { status: "created" | "replayed"; distribution: Source }
    | { status: "mismatch" }
  >
  replaceDistribution: (input: {
    id: string
    expectedVersion: number
    fields: { code: string; name: string }
  }) => Promise<
    { status: "updated"; distribution: Source } | { status: "missing" | "stale" }
  >
  deleteDistribution: (input: {
    id: string
    expectedVersion: number
  }) => Promise<
    { status: "deleted"; distribution: Source } | { status: "missing" | "stale" }
  >
}
type Env = { Variables: { collector: MaintenanceCollector; requestId: string } }

export function registerDistributionMaintenanceRoutes(
  app: Hono<Env>,
  dependencies: DistributionMaintenanceDependencies
) {
  app.get("/api/v1/maintenance/distributions", async (c) => {
    const input = parseCollection(c.req.url, false)
    if (input instanceof Response) return input
    const records = await dependencies.listDistributions({
      ...input,
      limit: input.limit + 1,
    })
    return c.json(page(records, input), 200)
  })
  app.post("/api/v1/maintenance/distributions", async (c) => {
    ...
    if (!key)
      return problem(
        ...
        "Idempotency-Key required",
        "Distribution create requires an Idempotency-Key header",
        c.req.path
      )
    if (key.length > 255)
      return problem(
    ...
    try {
      const result = await dependencies.createDistribution({
      ...
      const data = serialize(result.distribution)
      return c.json({ data }, 201, {
        ETag: data.etag,
        Location: `/api/v1/maintenance/distributions/${data.id}`,
      })
    } catch (error) {
      return mapError(error, "create", c.req.path)
    }
  })
  app.all("/api/v1/maintenance/distributions", (c) =>
    method(c.req.path, "GET, POST")
  )
  app.get("/api/v1/maintenance/distributions/options", async (c) => {
  ...
  app.all("/api/v1/maintenance/distributions/options", (c) =>
    method(c.req.path, "GET")
  )
  app.get("/api/v1/maintenance/distributions/:uuid", async (c) => {
    const id = c.req.param("uuid")
    if (!uuid(id)) return invalidId(c.req.path)
    const record = await dependencies.getDistribution(id)
    if (!record) return notFound(c.req.path)
    return c.json({ data: serialize(record) }, 200, { ETag: etag(record) })
  })
  app.put("/api/v1/maintenance/distributions/:uuid", async (c) => {
    const id = c.req.param("uuid")
    if (!uuid(id)) return invalidId(c.req.path)
    ...
    if (fields instanceof Response) return fields
    try {
      const result = await dependencies.replaceDistribution({
        id,
  ...
  app.delete("/api/v1/maintenance/distributions/:uuid", async (c) => {
    const id = c.req.param("uuid")
    if (!uuid(id)) return invalidId(c.req.path)
    const version = precondition(c.req.header("if-match"), id, c.req.path)
    if (version instanceof Response) return version
    try {
      const result = await dependencies.deleteDistribution({
        id,
        expectedVersion: version,
      })
  ...
  app.all("/api/v1/maintenance/distributions/:uuid", (c) =>
...
function parseCollection(url: string, optionsOnly: boolean): Input | Response {
  ...
  const parsed = (
    optionsOnly ? distributionOptionsInputSchema : distributionListInputSchema
  ).safeParse(raw)
...
async function parseBody(request: Request) {
  ...
  const parsed = distributionMutationBodySchema.safeParse(body)
  ...
  return problem(
    ...
    "The Distribution could not be saved",
    new URL(request.url).pathname,
    parsed.error.issues.map((issue) => {
      ...
      return {
        name: `/${field}`,
        code: `distribution_${field}_${failure}`,
        reason:
          failure === "required"
            ? `Distribution ${field === "code" ? "Code" : "Name"} must not be blank.`
            : failure === "too_long"
              ? `Distribution ${field === "code" ? "Code" : "Name"} must contain at most 255 characters.`
              : `Distribution ${field === "code" ? "Code" : "Name"} is invalid.`,
      }
    })
  )
}
function precondition(
  ...
  if (!value)
    return problem(
      ...
      "if_match_required",
      "If-Match required",
      "Distribution replacement and deletion require an If-Match header",
      instance
    )
  try {
    ...
    return problem(
      ...
      "invalid_if_match",
      "Invalid If-Match",
      "If-Match does not identify this Distribution version",
      instance
    )
  }
}
function mapError(
  ...
  if (constraint(error, "23505", "distribution_code_lower_unique_idx"))
    return problem(
      409,
      "distribution-code-conflict",
      "distribution_code_conflict",
      "Distribution Code already exists",
      "Another Distribution already uses this Distribution Code",
      instance
    )
  if (
    operation === "delete" &&
    constraint(error, "23001", "coin_distribution_id_distribution_id_fk")
  )
    return problem(
      409,
      "distribution-in-use",
      ...
      "Coins still use this Distribution, so it cannot be deleted",
      instance
    )
  throw error
}
...
function invalidId(instance: string) {
  return problem(
    400,
    "invalid-distribution-uuid",
    "invalid_distribution_uuid",
    "Invalid Distribution UUID",
    "Distribution UUID is invalid",
    instance
  )
}
function notFound(instance: string) {
  return problem(
    ...
    "Distribution not found",
    "No Distribution matches this UUID",
    instance
  )
}
function stale(instance: string) {
  return problem(
    412,
    "stale-distribution",
    "distribution_precondition_failed",
    "Distribution changed",
    "The Distribution changed after it was loaded; reload before retrying",
    instance
  )
}
