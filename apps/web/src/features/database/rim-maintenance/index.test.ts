...
describe("rim-maintenance public API", () => {
  ...
  it("keeps migrated Rim reads and writes outside the database package boundary", () => {
    for (const file of ["actions.ts", "rim-maintenance-route-data.ts"]) {
    ...
    expect(coinFormSource).toContain("maintenanceClient.rims.options")
