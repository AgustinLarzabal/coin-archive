...
function createApp(
  ...
  return createApiApp({
    ...
    listRims: async (input) =>
      rims.map((rim) => ({
        ...rim,
        cursorValue: rim[input.sort].toLowerCase(),
        cursorSecondaryValue:
          rim[input.sort === "name" ? "code" : "name"].toLowerCase(),
      })),
    getRim: async (id) => rims.find((rim) => rim.id === id) ?? null,
    createRim: async ({ fields }) => ({
    ...
    replaceRim: async ({ id, fields }) => ({
      status: "updated" as const,
      rim: { ...rims[0], id, ...fields, version: 2 },
    }),
    deleteRim: async () => ({
...
describe("protected Rim maintenance reads", () => {
  it("enforces authentication and the Editor/Admin policy", async () => {
    for (const [app, status, code] of [
      ...
      const response = await app.request(
        "https://api.coinarchive.app/api/v1/maintenance/rims"
      )
    ...
    }).request("https://api.coinarchive.app/api/v1/maintenance/rims")
    expect(adminResponse.status).toBe(200)
  })

  it("returns cursor-paginated Rims and compact options", async () => {
    const listRims = vi.fn(async () =>
      rims.map((rim) => ({
        ...rim,
        cursorValue: rim.name.toLowerCase(),
        cursorSecondaryValue: rim.code.toLowerCase(),
      }))
    )
    const app = createApp({ listRims })
    const list = await app.request(
      "https://api.coinarchive.app/api/v1/maintenance/rims?limit=1&q=reeded&sort=name&order=asc"
    )
    ...
    expect(listRims).toHaveBeenCalledWith({
      q: "reeded",
      limit: 2,
      sort: "name",
      order: "asc",
    ...
    const options = await app.request(
      "https://api.coinarchive.app/api/v1/maintenance/rims/options?q=reeded"
    )
    await expect(options.json()).resolves.toStrictEqual({
      data: rims.map(({ id, code, name }) => ({ id, code, name })),
      nextCursor: null,
    })
  })
  ...
  it("returns mutable detail with an opaque ETag", async () => {
    const app = createApp()
    const response = await app.request(
      `https://api.coinarchive.app/api/v1/maintenance/rims/${rims[0].id}`
    )

    expect(response.status).toBe(200)
    expect(response.headers.get("etag")).toMatch(/^"[A-Za-z0-9_-]+"$/)
    await expect(response.json()).resolves.toMatchObject({
      data: { id: rims[0].id, version: 1 },
    })

    const missing = await app.request(
      "https://api.coinarchive.app/api/v1/maintenance/rims/018f1a11-aaaa-7000-8000-000000000099"
    )
...
describe("protected Rim maintenance mutations", () => {
  ...
  it("creates with normalization, retry identity, and standard success headers", async () => {
    const createRim = vi.fn(async ({ fields }) => ({
      status: "created" as const,
      rim: { ...rims[0], ...fields },
    }))
    const response = await createApp({ createRim }).request(
      "https://api.coinarchive.app/api/v1/maintenance/rims",
      {
        method: "POST",
        headers: {
    ...
    expect(response.headers.get("location")).toBe(
      `/api/v1/maintenance/rims/${rims[0].id}`
    )
    expect(response.headers.get("etag")).toMatch(/^"[A-Za-z0-9_-]+"$/)
    expect(response.headers.get("cache-control")).toBe("private, no-store")
  ...
  it("returns authoritative pointer-addressed validation", async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({
      ...
      invalidParams: expect.arrayContaining([
        expect.objectContaining({
          name: "/code",
          code: "rim_code_required",
        }),
        expect.objectContaining({
          name: "/name",
          code: "rim_name_too_long",
        }),
      ]),
    })
    ...
    const invalidCode = await createApp().request(
      "https://api.coinarchive.app/api/v1/maintenance/rims",
      {
        ...
        body: JSON.stringify({ code: "Reeded Rim", name: "Reeded" }),
      }
    )
  ...
  it("replays identical creates and rejects mismatched key reuse", async () => {
    ...
    const mismatch = await createApp({
      createRim: async () => ({ status: "mismatch" }),
    }).request("https://api.coinarchive.app/api/v1/maintenance/rims", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  it("atomically replaces and deletes with opaque If-Match", async () => {
    const replaceRim = vi.fn(async ({ id, fields }) => ({
      status: "updated" as const,
      rim: { ...rims[0], id, ...fields, version: 2 },
    }))
    const deleteRim = vi.fn(async () => ({
      status: "deleted" as const,
      rim: rims[0],
    }))
    const app = createApp({ replaceRim, deleteRim })
  ...
  it("reports stale writes, Rim Code conflicts, and dependent-Coin conflicts", async () => {
    const stale = await createApp({
      replaceRim: async () => ({ status: "stale" }),
    }).request(
      `https://api.coinarchive.app/api/v1/maintenance/rims/${rims[0].id}`,
      {
        method: "PUT",
    ...
    await expect(stale.json()).resolves.toMatchObject({
      code: "rim_precondition_failed",
    })

    const staleDelete = await createApp({
      deleteRim: async () => ({ status: "stale" }),
    }).request(
      `https://api.coinarchive.app/api/v1/maintenance/rims/${rims[0].id}`,
      {
        method: "DELETE",
...
describe("Rim maintenance OpenAPI", () => {
  it("documents every Rim operation as Collector-protected", async () => {
    const response = await createApp().request(
    ...
    const collection = document.paths["/api/v1/maintenance/rims"]
    const detail = document.paths["/api/v1/maintenance/rims/{uuid}"]
    ...
    const rimOperations = JSON.stringify({ collection, detail })
    expect(rimOperations).toContain("rim_code_invalid")
    expect(rimOperations).toContain("rim_in_use")
  })
})
