...
import type { Mint, MintListInput } from "@coin-archive/api"
import type { Hono } from "hono"

import type { MaintenanceCollector } from "./orientation-maintenance"

type MintSourceRecord = Omit<Mint, "createdAt" | "updatedAt" | "etag"> & {
  createdAt: Date
  updatedAt: Date
}
type Cursor = { value: string; secondaryValue: string; id: string }
export type MintMaintenanceDependencies = {
  listMints: (
    input: Required<Pick<MintListInput, "limit" | "sort" | "order">> & {
      q?: string
      cursor?: Cursor
    }
  ) => Promise<
  ...
  >
  getMint: (id: string) => Promise<MintSourceRecord | null>
  createMint: (input: {
    collectorId: string
  ...
  replaceMint: (input: {
    ...
    | { status: "updated"; mint: MintSourceRecord }
  ...
  deleteMint: (input: {
    ...
    | { status: "deleted"; mint: MintSourceRecord }
    | { status: "missing" | "stale" }
  >
}
type Env = { Variables: { collector: MaintenanceCollector; requestId: string } }

export function registerMintMaintenanceRoutes(
  app: Hono<Env>,
  dependencies: MintMaintenanceDependencies
) {
  app.get("/api/v1/maintenance/mints", async (c) => {
    const input = parseCollection(c.req.url, false)
    if (input instanceof Response) return input
    const records = await dependencies.listMints({
      ...input,
      limit: input.limit + 1,
    })
    return c.json(page(records, input), 200)
  })
  app.post("/api/v1/maintenance/mints", async (c) => {
    ...
    if (!key)
      return problem(
        ...
        "Idempotency-Key required",
        "Mint create requires an Idempotency-Key header",
        c.req.path
      )
    const parsedHeaders = mintCreateInputSchema.shape.headers.safeParse({
      "idempotency-key": key,
    })
    ...
    const input = mintCreateInputSchema.parse({
      headers: parsedHeaders.data,
      body: fields,
    })
    try {
      const result = await dependencies.createMint({
      ...
      const data = serialize(result.mint)
      return c.json({ data }, 201, {
        ETag: data.etag,
        Location: `/api/v1/maintenance/mints/${data.id}`,
      })
    } catch (error) {
      return mapError(error, "create", c.req.path)
    }
  })
  app.all("/api/v1/maintenance/mints", (c) => method(c.req.path, "GET, POST"))
  app.get("/api/v1/maintenance/mints/options", async (c) => {
  ...
  app.all("/api/v1/maintenance/mints/options", (c) => method(c.req.path, "GET"))
  app.get("/api/v1/maintenance/mints/:uuid", async (c) => {
    const id = c.req.param("uuid")
    if (!mintDetailInputSchema.safeParse({ uuid: id }).success)
      return invalidId(c.req.path)
    const record = await dependencies.getMint(id)
    if (!record) return notFound(c.req.path)
    return c.json({ data: serialize(record) }, 200, { ETag: etag(record) })
  })
  app.put("/api/v1/maintenance/mints/:uuid", async (c) => {
    const id = c.req.param("uuid")
    if (!mintReplaceInputSchema.shape.params.safeParse({ uuid: id }).success)
      return invalidId(c.req.path)
    const version = precondition(c.req.header("if-match"), id, c.req.path)
    if (version instanceof Response) return version
    const fields = await parseBody(c.req.raw)
    if (fields instanceof Response) return fields
    const input = mintReplaceInputSchema.parse({
      params: { uuid: id },
      headers: { "if-match": c.req.header("if-match") },
      body: fields,
    })
    try {
      const result = await dependencies.replaceMint({
  ...
  app.delete("/api/v1/maintenance/mints/:uuid", async (c) => {
    const id = c.req.param("uuid")
    if (!mintDeleteInputSchema.shape.params.safeParse({ uuid: id }).success)
      return invalidId(c.req.path)
    const version = precondition(c.req.header("if-match"), id, c.req.path)
    if (version instanceof Response) return version
    const input = mintDeleteInputSchema.parse({
      params: { uuid: id },
      headers: { "if-match": c.req.header("if-match") },
    })
    try {
      const result = await dependencies.deleteMint({
        id: input.params.uuid,
        expectedVersion: version,
      })
  ...
  app.all("/api/v1/maintenance/mints/:uuid", (c) =>
    method(c.req.path, "GET, PUT, DELETE")
  )
}

type MintCollectionInput = {
  q?: string
  cursor?: Cursor
  limit: number
  sort: "code" | "name"
...
function parseCollection(
  ...
  const parsed = (
    optionsOnly ? mintOptionsInputSchema : mintListInputSchema
  ).safeParse(raw)
  if (!parsed.success) return invalidQuery(requestUrl.pathname)
  const sort = optionsOnly
      ? "name"
      : ((raw.sort as MintCollectionInput["sort"] | undefined) ?? "name"),
    order = optionsOnly
      ? "asc"
      : ((raw.order as MintCollectionInput["order"] | undefined) ?? "asc")
  const cursor =
...
function page(
  records: (MintSourceRecord & {
    cursorValue: string
    cursorSecondaryValue: string
  })[],
  input: Pick<MintCollectionInput, "limit" | "sort" | "order">
) {
...
async function parseBody(request: Request) {
  ...
  const parsed = mintMutationBodySchema.safeParse(body)
    ...
    parsed.error.issues.map((issue) => {
      ...
      if (pathField !== "code" && pathField !== "name") {
        return {
          ...
          reason: "The Mint request body must be an object.",
        }
      }
      ...
      return {
        name: `/${field}`,
        code: `mint_${field}_${failure}`,
        reason:
          failure === "required"
            ? `Mint ${field === "code" ? "Code" : "Name"} must not be blank.`
            : failure === "too_long"
              ? `Mint ${field === "code" ? "Code" : "Name"} must contain at most 255 characters.`
              : `Mint ${field === "code" ? "Code" : "Name"} is invalid.`,
      }
    })
  )
}
function precondition(
  ...
  if (!value)
    return problem(
      ...
      "if_match_required",
      "If-Match required",
      "Mint replacement and deletion require an If-Match header",
      instance
    )
  try {
    ...
    return problem(
      ...
      "invalid_if_match",
      "Invalid If-Match",
      "If-Match does not identify this Mint version",
      instance
    )
  }
}
function mapError(
  ...
  if (constraint(error, "23505", "mint_code_lower_unique_idx"))
    return problem(
      409,
      "mint-code-conflict",
      "mint_code_conflict",
      "Mint Code already exists",
      "Another Mint already uses this Mint Code",
      instance
    )
  if (
    operation === "delete" &&
    constraint(error, "23001", "coin_mint_mint_id_mint_id_fk")
  )
    return problem(
      409,
      "mint-in-use",
      "mint_in_use",
      "Mint is in use",
      "Coin Mint Attributions still use this Mint, so it cannot be deleted",
      instance
    )
  throw error
}
...
function stale(instance: string) {
  return problem(
    ...
    "Mint changed",
    "The Mint changed after it was loaded; reload before retrying",
    instance
  )
}
...
function etag(record: Pick<MintSourceRecord, "id" | "version">) {
