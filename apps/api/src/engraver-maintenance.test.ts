...
function createApp(
  ...
  return createApiApp({
    ...
    listEngravers: async (input) =>
      engravers.map((engraver) => ({
        ...engraver,
        cursorValue: engraver[input.sort].toLowerCase(),
        cursorSecondaryValue:
          engraver[input.sort === "name" ? "code" : "name"].toLowerCase(),
      })),
    getEngraver: async (id) => engravers.find((engraver) => engraver.id === id) ?? null,
    createEngraver: async ({ fields }) => ({
    ...
    replaceEngraver: async ({ id, fields }) => ({
      status: "updated" as const,
      engraver: { ...engravers[0], id, ...fields, version: 2 },
    }),
    deleteEngraver: async () => ({
      status: "deleted" as const,
      engraver: engravers[0],
    }),
    ...overrides,
  })
}

describe("protected Engraver maintenance reads", () => {
  it("enforces authentication and the Editor/Admin policy", async () => {
    for (const [app, status, code] of [
      ...
      const response = await app.request(
        "https://api.coinarchive.app/api/v1/maintenance/engravers"
      )
    ...
    }).request("https://api.coinarchive.app/api/v1/maintenance/engravers")
    expect(adminResponse.status).toBe(200)
  })

  it("returns cursor-paginated Engravers and compact options", async () => {
    const listEngravers = vi.fn(async () =>
      engravers.map((engraver) => ({
        ...engraver,
        cursorValue: engraver.name.toLowerCase(),
        cursorSecondaryValue: engraver.code.toLowerCase(),
      }))
    )
    const app = createApp({ listEngravers })
    const list = await app.request(
      "https://api.coinarchive.app/api/v1/maintenance/engravers?limit=1&q=reeded&sort=name&order=asc"
    )
    ...
    expect(listEngravers).toHaveBeenCalledWith({
      q: "reeded",
      limit: 2,
      sort: "name",
      order: "asc",
    ...
    const options = await app.request(
      "https://api.coinarchive.app/api/v1/maintenance/engravers/options?q=reeded"
    )
    await expect(options.json()).resolves.toStrictEqual({
      data: engravers.map(({ id, code, name }) => ({ id, code, name })),
      nextCursor: null,
    })
  })

  it("round-trips Unicode Engraver names through opaque cursors", async () => {
    const unicodeEngraver = { ...engravers[0], name: "円形" }
    const listEngravers = vi
      .fn()
      .mockResolvedValueOnce([
        {
          ...unicodeEngraver,
          cursorValue: unicodeEngraver.name,
          cursorSecondaryValue: unicodeEngraver.code,
        },
        {
          ...engravers[1],
          cursorValue: engravers[1].name.toLowerCase(),
          cursorSecondaryValue: engravers[1].code,
        },
      ])
      .mockResolvedValueOnce([])
    const app = createApp({ listEngravers })
    const firstResponse = await app.request(
      "https://api.coinarchive.app/api/v1/maintenance/engravers?limit=1"
    )
    ...
    const secondResponse = await app.request(
      `https://api.coinarchive.app/api/v1/maintenance/engravers?limit=1&cursor=${firstPage.nextCursor}`
    )

    expect(secondResponse.status).toBe(200)
    expect(listEngravers).toHaveBeenLastCalledWith({
      cursor: {
        value: unicodeEngraver.name,
        secondaryValue: unicodeEngraver.code,
        id: unicodeEngraver.id,
      },
      limit: 2,
      sort: "name",
      order: "asc",
  ...
  it("returns mutable detail with an opaque ETag", async () => {
    const app = createApp()
    const response = await app.request(
      `https://api.coinarchive.app/api/v1/maintenance/engravers/${engravers[0].id}`
    )

    expect(response.status).toBe(200)
    expect(response.headers.get("etag")).toMatch(/^"[A-Za-z0-9_-]+"$/)
    await expect(response.json()).resolves.toMatchObject({
      data: { id: engravers[0].id, version: 1 },
    })

    const missing = await app.request(
      "https://api.coinarchive.app/api/v1/maintenance/engravers/018f1a11-aaaa-7000-8000-000000000099"
    )
    expect(missing.status).toBe(404)
    await expect(missing.json()).resolves.toMatchObject({
      code: "engraver_not_found",
    })
  })
...
describe("protected Engraver maintenance mutations", () => {
  ...
  it("creates with normalization, retry identity, and standard success headers", async () => {
    const createEngraver = vi.fn(async ({ fields }) => ({
      status: "created" as const,
      engraver: { ...engravers[0], ...fields },
    }))
    const response = await createApp({ createEngraver }).request(
      "https://api.coinarchive.app/api/v1/maintenance/engravers",
      {
        method: "POST",
        headers: {
    ...
    expect(response.headers.get("location")).toBe(
      `/api/v1/maintenance/engravers/${engravers[0].id}`
    )
    expect(response.headers.get("etag")).toMatch(/^"[A-Za-z0-9_-]+"$/)
    expect(response.headers.get("cache-control")).toBe("private, no-store")
  ...
  it("returns authoritative pointer-addressed validation", async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({
      ...
      invalidParams: expect.arrayContaining([
        expect.objectContaining({
          name: "/code",
          code: "engraver_code_required",
        }),
        expect.objectContaining({
          name: "/name",
          code: "engraver_name_too_long",
        }),
      ]),
    })
    ...
    const invalidCode = await createApp().request(
      "https://api.coinarchive.app/api/v1/maintenance/engravers",
      {
        ...
        body: JSON.stringify({ code: "Reeded Engraver", name: "Reeded" }),
      }
    )
  ...
  it("replays identical creates and rejects mismatched key reuse", async () => {
    ...
    const mismatch = await createApp({
      createEngraver: async () => ({ status: "mismatch" }),
    }).request("https://api.coinarchive.app/api/v1/maintenance/engravers", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  it("atomically replaces and deletes with opaque If-Match", async () => {
    const replaceEngraver = vi.fn(async ({ id, fields }) => ({
      status: "updated" as const,
      engraver: { ...engravers[0], id, ...fields, version: 2 },
    }))
    const deleteEngraver = vi.fn(async () => ({
      status: "deleted" as const,
      engraver: engravers[0],
    }))
    const app = createApp({ replaceEngraver, deleteEngraver })
  ...
  it("reports stale writes, Engraver Code conflicts, and Engraver Attribution conflicts", async () => {
    const stale = await createApp({
      replaceEngraver: async () => ({ status: "stale" }),
    }).request(
      `https://api.coinarchive.app/api/v1/maintenance/engravers/${engravers[0].id}`,
      {
        method: "PUT",
    ...
    await expect(stale.json()).resolves.toMatchObject({
      code: "engraver_precondition_failed",
    })

    const staleDelete = await createApp({
      deleteEngraver: async () => ({ status: "stale" }),
    }).request(
      `https://api.coinarchive.app/api/v1/maintenance/engravers/${engravers[0].id}`,
      {
        method: "DELETE",
...
describe("Engraver maintenance OpenAPI", () => {
  it("documents every Engraver operation as Collector-protected", async () => {
    const response = await createApp().request(
    ...
    const collection = document.paths["/api/v1/maintenance/engravers"]
    const detail = document.paths["/api/v1/maintenance/engravers/{uuid}"]
    ...
    const engraverOperations = JSON.stringify({ collection, detail })
    expect(engraverOperations).toContain("engraver_code_invalid")
    expect(engraverOperations).toContain("engraver_in_use")
    expect(engraverOperations).toContain("engraver_not_found")
    expect(engraverOperations).toContain("invalid_engraver_uuid")
  })
})
