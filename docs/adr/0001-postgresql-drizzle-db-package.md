...
Coin Archive will use PostgreSQL as its source-of-truth database and Drizzle as its TypeScript schema and query layer. Database schema, migrations, seed logic, connection setup, and shared query functions will live in `packages/db`, with migrations stored in a `migrations` folder, so applications consume database behavior through a single workspace package instead of owning schema details themselves.
