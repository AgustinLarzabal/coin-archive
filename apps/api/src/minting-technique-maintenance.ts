...
type MintingTechniqueSourceRecord = Omit<
  MintingTechnique,
...
export type MintingTechniqueMaintenanceDependencies = {
  listMintingTechniques: (
    input: Required<
      Pick<MintingTechniqueListInput, "limit" | "sort" | "order">
    > & {
      q?: string
      cursor?: Cursor
    }
  ) => Promise<
    (MintingTechniqueSourceRecord & {
      cursorValue: string
  ...
  getMintingTechnique: (
    id: string
  ) => Promise<MintingTechniqueSourceRecord | null>
  createMintingTechnique: (input: {
    collectorId: string
    ...
    | {
        ...
        mintingTechnique: MintingTechniqueSourceRecord
      }
    | { status: "mismatch" }
  >
  replaceMintingTechnique: (input: {
    id: string
    expectedVersion: number
    fields: { code: string; name: string }
  }) => Promise<
    | { status: "updated"; mintingTechnique: MintingTechniqueSourceRecord }
    | { status: "missing" | "stale" }
  >
  deleteMintingTechnique: (input: {
    id: string
    expectedVersion: number
  }) => Promise<
    | { status: "deleted"; mintingTechnique: MintingTechniqueSourceRecord }
    | { status: "missing" | "stale" }
  >
}
type Env = { Variables: { collector: MaintenanceCollector; requestId: string } }

export function registerMintingTechniqueMaintenanceRoutes(
  app: Hono<Env>,
  dependencies: MintingTechniqueMaintenanceDependencies
) {
  app.get("/api/v1/maintenance/minting-techniques", async (c) => {
    const input = parseCollection(c.req.url, false)
    if (input instanceof Response) return input
    const records = await dependencies.listMintingTechniques({
      ...input,
      limit: input.limit + 1,
    })
    return c.json(page(records, input), 200)
  })
  app.post("/api/v1/maintenance/minting-techniques", async (c) => {
    ...
    if (!key)
      return problem(
        ...
        "Idempotency-Key required",
        "Minting Technique create requires an Idempotency-Key header",
        c.req.path
      )
    const parsedHeaders =
      mintingTechniqueCreateInputSchema.shape.headers.safeParse({
        "idempotency-key": key,
      })
    ...
    const input = mintingTechniqueCreateInputSchema.parse({
      headers: parsedHeaders.data,
      body: fields,
    })
    try {
      const result = await dependencies.createMintingTechnique({
      ...
      const data = serialize(result.mintingTechnique)
      return c.json({ data }, 201, {
        ETag: data.etag,
        Location: `/api/v1/maintenance/minting-techniques/${data.id}`,
      })
    } catch (error) {
      return mapError(error, "create", c.req.path)
    }
  })
  app.all("/api/v1/maintenance/minting-techniques", (c) =>
    method(c.req.path, "GET, POST")
  )
  app.get("/api/v1/maintenance/minting-techniques/options", async (c) => {
  ...
  app.all("/api/v1/maintenance/minting-techniques/options", (c) =>
    method(c.req.path, "GET")
  )
  app.get("/api/v1/maintenance/minting-techniques/:uuid", async (c) => {
    const id = c.req.param("uuid")
    if (!mintingTechniqueDetailInputSchema.safeParse({ uuid: id }).success)
      return invalidId(c.req.path)
    const record = await dependencies.getMintingTechnique(id)
    if (!record) return notFound(c.req.path)
    return c.json({ data: serialize(record) }, 200, { ETag: etag(record) })
  })
  app.put("/api/v1/maintenance/minting-techniques/:uuid", async (c) => {
    const id = c.req.param("uuid")
    if (
      !mintingTechniqueReplaceInputSchema.shape.params.safeParse({ uuid: id })
        .success
    )
      return invalidId(c.req.path)
    ...
    const fields = await parseBody(c.req.raw)
    if (fields instanceof Response) return fields
    const input = mintingTechniqueReplaceInputSchema.parse({
      params: { uuid: id },
      headers: { "if-match": c.req.header("if-match") },
      body: fields,
    })
    try {
      const result = await dependencies.replaceMintingTechnique({
  ...
  app.delete("/api/v1/maintenance/minting-techniques/:uuid", async (c) => {
    const id = c.req.param("uuid")
    if (
      !mintingTechniqueDeleteInputSchema.shape.params.safeParse({ uuid: id })
        .success
    )
      return invalidId(c.req.path)
    const version = precondition(c.req.header("if-match"), id, c.req.path)
    if (version instanceof Response) return version
    const input = mintingTechniqueDeleteInputSchema.parse({
      params: { uuid: id },
      headers: { "if-match": c.req.header("if-match") },
    })
    try {
      const result = await dependencies.deleteMintingTechnique({
        id: input.params.uuid,
        expectedVersion: version,
      })
  ...
  app.all("/api/v1/maintenance/minting-techniques/:uuid", (c) =>
    method(c.req.path, "GET, PUT, DELETE")
  )
}

type MintingTechniqueCollectionInput = {
  q?: string
  cursor?: Cursor
  limit: number
  sort: "code" | "name"
...
function parseCollection(
  url: string,
  optionsOnly: boolean
): MintingTechniqueCollectionInput | Response {
  ...
  const sort = optionsOnly
      ? "name"
      : ((raw.sort as MintingTechniqueCollectionInput["sort"] | undefined) ??
        "name"),
    order = optionsOnly
      ? "asc"
      : ((raw.order as MintingTechniqueCollectionInput["order"] | undefined) ??
        "asc")
  const cursor =
...
function page(
  records: (MintingTechniqueSourceRecord & {
    cursorValue: string
    cursorSecondaryValue: string
  })[],
  input: Pick<MintingTechniqueCollectionInput, "limit" | "sort" | "order">
) {
...
function serialize(
  record: MintingTechniqueSourceRecord & {
    cursorValue?: string
    cursorSecondaryValue?: string
  }
): MintingTechnique {
...
async function parseBody(request: Request) {
  ...
  const parsed = mintingTechniqueMutationBodySchema.safeParse(body)
  if (parsed.success) return parsed.data
  return problem(
    422,
    "minting-technique-validation",
    "minting_technique_validation_failed",
    "Minting Technique validation failed",
    "The Minting Technique could not be saved",
    new URL(request.url).pathname,
    parsed.error.issues.map((issue) => {
      ...
      if (pathField !== "code" && pathField !== "name") {
        return {
          ...
          reason: "The Minting Technique request body must be an object.",
        }
      }
      ...
      return {
        name: `/${field}`,
        code: `minting_technique_${field}_${failure}`,
        reason:
          failure === "required"
            ? `Minting Technique ${field === "code" ? "Code" : "Name"} must not be blank.`
            : failure === "too_long"
              ? `Minting Technique ${field === "code" ? "Code" : "Name"} must contain at most 255 characters.`
              : `Minting Technique ${field === "code" ? "Code" : "Name"} is invalid.`,
      }
    })
  )
}
function precondition(
  ...
  if (!value)
    return problem(
      ...
      "if_match_required",
      "If-Match required",
      "Minting Technique replacement and deletion require an If-Match header",
      instance
    )
  try {
    ...
    return problem(
      ...
      "invalid_if_match",
      "Invalid If-Match",
      "If-Match does not identify this Minting Technique version",
      instance
    )
  }
}
function mapError(
  ...
  if (constraint(error, "23505", "technique_code_lower_unique_idx"))
    return problem(
      409,
      "minting-technique-code-conflict",
      "minting_technique_code_conflict",
      "Minting Technique Code already exists",
      "Another Minting Technique already uses this Minting Technique Code",
      instance
    )
  if (
    operation === "delete" &&
    constraint(error, "23001", "coin_technique_id_technique_id_fk")
  )
    return problem(
      409,
      "minting-technique-in-use",
      "minting_technique_in_use",
      "Minting Technique is in use",
      "Coins still use this Minting Technique, so it cannot be deleted",
      instance
    )
  throw error
}
...
function invalidId(instance: string) {
  return problem(
    400,
    "invalid-minting-technique-uuid",
    "invalid_minting_technique_uuid",
    "Invalid Minting Technique UUID",
    "Minting Technique UUID is invalid",
    instance
  )
}
function notFound(instance: string) {
  return problem(
    404,
    "minting-technique-not-found",
    "minting_technique_not_found",
    "Minting Technique not found",
    "No Minting Technique matches this UUID",
    instance
  )
}
...
function etag(record: Pick<MintingTechniqueSourceRecord, "id" | "version">) {
